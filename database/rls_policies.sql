...
-- ============================================================================
-- This file contains RLS policies to enforce tenant isolation across all
-- clinical tables. Each policy restricts access based on the current
-- session's tenant_id setting.
...
COMMENT ON FUNCTION set_tenant(uuid) IS 'Sets the current tenant_id for the session to enforce RLS policies';
COMMENT ON FUNCTION get_current_tenant_id() IS 'Gets the current tenant_id from the session';
COMMENT ON TABLE audit_logs IS 'Audit trail for all changes to clinical data with tenant isolation';
COMMENT ON FUNCTION audit_trigger_function() IS 'Trigger function that logs all DML operations on clinical tables';